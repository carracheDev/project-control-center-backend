...
export async function loginTestUser(app: INestApplication<App>, email: string): Promise<E2eAgent> {
  const agent = request.agent(app.getHttpServer());
  await agent.post('/auth/login').send({ email, password }).expect(201);
  return agent;
}

export async function createTestAuthContext(app: INestApplication<App>, prisma: PrismaService): Promise<E2eAuthContext> {
...
export async function cleanupTestAuthContext(app: INestApplication<App>, prisma: PrismaService, auth: E2eAuthContext): Promise<void> {
  await prisma.project.deleteMany();
  await prisma.user.deleteMany({ where: { id: { in: auth.userIds } } });
  await app.close();
}

export function configureE2eApp(app: INestApplication<App>): void {
